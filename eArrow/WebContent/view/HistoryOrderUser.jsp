	...
	<body>
		...
		<%
			...
			for(int i = 0; i < listaOrdini.size(); i++) {
				int idOrdine = listaOrdini.get(i).getId();
				...
				UtenteBean utente = UtenteDao.doRetrievebyId(id);
				
				Date data = listaOrdini.get(i).getData();
				
				HashMap<Integer, Integer> ordineHash = ComposizioneOrdineDAO.doRetrieveIdbyUserOrderId(listaOrdini.get(i).getId());
		%>
		...
		<div class="card-body">
			...
			<aside class="col-md-3" id="imageContainer">
				<a class="img-wrap img-fluid text-none">
					...
					<label class="titleSelect">ID Ordine: <%=listaOrdini.get(i).getId()%></label>
				</a>
			</aside>
